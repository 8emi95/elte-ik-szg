...
//
glm::vec3 CMyApp::GetUV_Sphere(float u, float v)
{
...
glm::vec3 CMyApp::GetUV_Cylinder(float u, float v) {
	u *= -2 * M_PI;
	v *= H;
	float r = 2.0;
	float cu = cosf(u),
...
glm::vec3 CMyApp::GetUV_Torus(float u, float v) {
	u *= 2 * M_PI;
	v *= 2 * M_PI;
	float r_small = 1.0,
		  r_big = 2.0,
		...
		cv = cosf(v),
		sv = sinf(v);
	return glm::vec3((r_big+r_small*cu)*cv,
					r_small*su,
					(r_big + r_small * cu)*sv);
}
glm::vec3 CMyApp::GetUV_Klein(float u, float v) {
	u *= 2 * M_PI;
	v *= 2 * M_PI;
	float r = 4.0*(1.0-(cosf(u)/2.0));
	float cu = cosf(u),
		su = sinf(u),
		cv = cosf(v),
		sv = sinf(v),
	...
	if (v < M_PI)
	{
		return glm::vec3( a*cu*(1+su)+r*cu*cv,
						  r*sv,
						  b*su+r*su*cv);
	}
	else
	{
		return glm::vec3( a*cu*(1 + su) + r * cosf(v+M_PI),
			              r*sv,
						  b*su );
	}
}
glm::vec3 CMyApp::GetUV_Mobius(float u, float v) {
	//GL_CULL_FACE kikapcsolása, hogy látszódjon
	u *= 2 * M_PI;
	v = v*2-1;
	float cu = cosf(u),
		su = sinf(u),
		c_half_u = cosf(0.5*u),
		s_half_u = sinf(0.5*u);
	return glm::vec3((1+0.5*v*c_half_u)*cu,
		0.5*v*s_half_u,
		(1 + 0.5*v*c_half_u)*su);
}
...
bool CMyApp::Init()
{
	...
	for (int i=0; i<=N; ++i)
		for (int j=0; j<=M; ++j)
		{
			...
			// vert[i + j * (N + 1)].p = GetUV_Cylinder(u, v);
			// vert[i + j * (N + 1)].p = GetUV_Mobius(u, v);
			// vert[i + j * (N + 1)].p = GetUV_Sphere(u, v);
			// vert[i + j * (N + 1)].p = GetUV_Torus(u, v);
			vert[i + j*(N+1)].p = GetUV_Klein(u, v);
...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3( 20*cosf(t), 20,  20*sinf(t)),		// honnan nézzük a színteret
