...
void CMyApp::DrawMesh()
{
	...
	ImGui::DragFloat("elforagatas", &angle ,0.001f);

	m_matWorld = glm::translate<float>( glm::vec3(0, 1, 0) ) * glm::rotate<float>(angle, glm::vec3(0,1,0));
	glm::mat4 m_matWorldIT = glm::transpose(glm::inverse(m_matWorld));
...
void CMyApp::Render()
{
	...
	static int asd = 1;
	ImGui::DragInt("my int", &asd);

	static std::string sz;
	ImGui::InputText("szoveg", sz);
