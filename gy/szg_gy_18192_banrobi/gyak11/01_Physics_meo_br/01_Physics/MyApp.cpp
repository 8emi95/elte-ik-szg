...
bool CMyApp::Init()
{
	...
	*/
	...
	m_gpuParticleVelBuffer = m_particleVel;
...
void CMyApp::Update()
{
	...
	static float energyRemaining = 1;	// tökéletesen rugalmas ütközés
	static float gravityMultiplier = 1;
	static glm::vec3 gravityDir = glm::vec3(0, -1, 0);

	ImGui::SliderInt("part_count", &m_particleCount, 1, 100);
	ImGui::DragFloat("gravity", &gravityMultiplier, 0.01f, 0.0f, 10.f);
	ImGui::SliderFloat("energy", &energyRemaining, 0.0f, 1.0f);
	if (ImGui::SliderFloat3("gravityDirection", &gravityDir.x, -1.0f, 1.0f)) {
		gravityDir = glm::normalize(gravityDir);
	}
	if (ImGui::Button("teszt gomb")) {
		std::cout << "ne nyomogass!" << std::endl;
	}

	glm::vec3 deltaVel = gravityDir * 10.0f*gravityMultiplier * delta_time;

	for (int i = 0; i < m_particleCount; ++i)
	{
		m_particlePos[i] += m_particleVel[i] * delta_time;

		m_particleVel[i] += deltaVel;
