...
glm::vec3	CMyApp::GetTorusUV(float u, float v)
{
	u *= 2 * 3.1415f;
	v *= 2 * 3.1415f;
	float cu = cosf(u), su = sinf(u), cv = cosf(v), sv = sinf(v);
...
bool CMyApp::Init()
{
	...
	//glPolygonMode(GL_FRONT, GL_LINE); // MEO ----------------------------------------------------------------------------------
	...
	for (int i=0; i<=N; ++i)
		for (int j=0; j<=M; ++j)
		{
			...
			vert[i + j*(N+1)].p = GetTorusUV(u, v); // MEO ------------------------------------------------------------------------
...
void CMyApp::Update()
{
	...
	float t = SDL_GetTicks()/1000.0f;	// glm::vec3( 5*cosf(t),  5,  5*sinf(t))
	m_matView = glm::lookAt(glm::vec3( 40*cosf(t), 40,  40*sinf(t)),		// honnan nézzük a színteret // MEO -------------------------------------------------
...
void CMyApp::Render()
{
	...
	*/
	...
	float t = SDL_GetTicks() / 1000.0 * 2 * M_PI / 15.0;
	for (int i = 0; i < k; ++i) {

		m_matWorld = 
			glm::rotate<float>(t + float(i) / k * 2 * M_PI, glm::vec3(0, 1, 0))
			*glm::translate<float>(glm::vec3(0, 0, 20))
