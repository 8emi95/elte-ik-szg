...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3( 5.0,  5.0, 0.0),		// honnan nézzük a színteret
							glm::vec3( 0,  0,  0),		// a színtér melyik pontját nézzük
							glm::vec3( 0,  1,  0));		// felfelé mutató irány a világban
							
    
    float length=6.0+2.0*sin(t);
    
    float angle1=t/1.1;
    float angle2=t/0.8;
    
    light_pos[0]=length*cos(angle2);
    light_pos[1]=length*sin(angle2)*cos(angle1);
    light_pos[2]=length*sin(angle2)*sin(angle1);
}
...
void CMyApp::Render()
{
	...
	*/
    ...
    glUniform3f(m_loc_light_pos,light_pos[0],light_pos[1],light_pos[2]);
