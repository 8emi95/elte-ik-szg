...
bool CMyApp::Init()
{
	...
	// a második attribútumhoz pedig a VBO-ban sizeof(Vertex) ugrás után sizeof(glm::vec3)-nyit menve újabb 3 float adatot találunk (normálvektor)
	glEnableVertexAttribArray(1); // ez lesz majd a normálvektor
	glVertexAttribPointer(
		1,
		3, 
		GL_FLOAT,
	...
	// textúrakoordináták bekapcsolása a 2-es azonosítójú attribútum csatornán
...
void CMyApp::Render()
{
	...
	*/
	...
	glBindTexture(GL_TEXTURE_2D, m_loadedTextureID);
	glBindSampler(0 /*hw samplerindex*/, m_samplerID);
	glUniform1i(m_loc_tex, 0 /*hw samplerindex*/);

	glActiveTexture(GL_TEXTURE0 + 1 /*hw samplerindex*/);
	glBindTexture(GL_TEXTURE_2D, m_generatedTextureID);
	glBindSampler(1 /*hw samplerindex*/, m_samplerID);
	glUniform1i(m_loc_tex2, 1 /*hw samplerindex*/);
	
	float t = 0.5 + 0.5*sin(SDL_GetTicks() / 1000.0*M_PI);
