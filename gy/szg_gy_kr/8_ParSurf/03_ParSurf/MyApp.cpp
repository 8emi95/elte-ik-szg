...
bool CMyApp::Init()
{
	...
	for (int i=0; i<=N; ++i)
		for (int j=0; j<=M; ++j)
		{
			...
			vert[i + j * (N + 1)].p = GetPlanePos(u, v);
			vert[i + j*(N + 1)].c = glm::vec3(0,1,0);
...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3(0, 6, 12),		// honnan nézzük a színteret
		glm::vec3(0, 0, 0),		// a színtér melyik pontját nézzük
		glm::vec3(0, 1, 0));		// felfelé mutató irány a világban
}
...
void CMyApp::Render()
{
	...
	m_matWorld = glm::scale(glm::vec3(5, 5, 5));
