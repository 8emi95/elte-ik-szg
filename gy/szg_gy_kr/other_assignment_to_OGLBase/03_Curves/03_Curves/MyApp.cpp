...
bool CMyApp::Init()
{
	...
	m_textureShell.FromFile("shell.jpg");
	m_textureEarth.FromFile("earth.png");
...
glm::vec3	CMyApp::GetConePos(float u, float v)
{
	...
	float mul = (h - u) / h;
	return glm::vec3(mul*r*cosf(v), u, mul*r*sinf(v));
}


void CMyApp::initDisc()
{
	...
	for (int i = 0; i <= N; ++i)
		for (int j = 0; j <= M; ++j)
		{
			...
			glm::vec3 p = GetDiscPos(u, v);
			pos[i + j * (N + 1)] = p;
			normal[i + j * (N + 1)] = glm::vec3(0,1,0);
			tex[i + j * (N + 1)] = glm::vec2(0.5*p.x+0.5, 0.5*p.z+0.5);
		}
...
void CMyApp::initCone()
{
	...
	for (int i = 0; i <= N; ++i)
		for (int j = 0; j <= M; ++j)
		{
			...
			glm::vec3 p = GetConePos(u, v);
			pos[i + j * (N + 1)] = p;
		
			glm::vec3 du = GetConePos(u + 0.01, v) - GetConePos(u - 0.01, v);
			glm::vec3 dv = GetConePos(u, v + 0.01) - GetConePos(u, v - 0.01);
			normal[i + j * (N + 1)] = glm::cross(du,dv);
...
void CMyApp::initCylinder() {
	...
	for (int i = 0; i <= N; ++i)
		for (int j = 0; j <= M; ++j)
		{
			...
			auto p = GetCylinderPos(u, v);
			pos[i + j * (N + 1)] = p;
			normal[i + j * (N + 1)] = glm::vec3(p.x, 0, p.z);
			tex[i + j * (N + 1)] = glm::vec2(u, v);
		}
	m_gpuBufferPos.BufferData(pos);
	m_gpuBufferNormal.BufferData(normal);
...
glm::vec3 CMyApp::GetHalfSpherePos(float u, float v)
{
	u *= 2*3.1415f;
	v *= 3.1415f/2;
	float cu = cosf(u), su = sinf(u), cv = cosf(v), sv = sinf(v);
...
void CMyApp::initHalfSphere() {
	...
	for (int i = 0; i <= N; ++i)
		for (int j = 0; j <= M; ++j)
		{
			...
			pos[i + j * (N + 1)] = GetHalfSpherePos(u, v);
			normal[i + j * (N + 1)] = glm::normalize(GetHalfSpherePos(u, v));
			tex[i + j * (N + 1)] = glm::vec2(u, 1-v);
		}
	m_gpuBufferPos.BufferData(pos);
	m_gpuBufferNormal.BufferData(normal);
	m_gpuBufferTex.BufferData(tex);
...
void  CMyApp::renderClosedHalfSphere(const glm::mat4 &world, const Texture2D &tex)
{
	renderDisc(world*glm::rotate<float>(M_PI, glm::vec3(0,0,1)), tex);
	renderHalfSphere(world, tex);
}
...
void CMyApp::renderCylinder(const glm::mat4 &world, const Texture2D &tex)
{
	m_program.Use();
	...
	m_vaoCylinder.Unbind(); // nem feltétlen szükséges: a m_mesh->draw beállítja a saját VAO-ját

	m_program.Unuse();
}


void CMyApp::renderHalfSphere(const glm::mat4 &world, const Texture2D &tex)
{
	m_program.Use();
	...
	m_vaoHalfSphere.Bind();
	glDrawElements(GL_TRIANGLES, 6 * N*M, GL_UNSIGNED_INT, 0);
	m_vaoHalfSphere.Unbind(); // nem feltétlen szükséges: a m_mesh->draw beállítja a saját VAO-ját

	m_program.Unuse();
}


void CMyApp::renderDisc(const glm::mat4 &world, const Texture2D &tex)
{
	m_program.Use();
	...
	m_vaoDisc.Bind();
	glDrawElements(GL_TRIANGLES, 6 * N*M, GL_UNSIGNED_INT, 0);
	m_vaoDisc.Unbind(); // nem feltétlen szükséges: a m_mesh->draw beállítja a saját VAO-ját

	m_program.Unuse();
}


void CMyApp::renderCone(const glm::mat4 &world, const Texture2D &tex)
{
	m_program.Use();
	...
	m_vaoCone.Bind();
	glDrawElements(GL_TRIANGLES, 6 * N*M, GL_UNSIGNED_INT, 0);
	m_vaoCone.Unbind(); // nem feltétlen szükséges: a m_mesh->draw beállítja a saját VAO-ját

	m_program.Unuse();
}

void CMyApp::renderClosedCone(const glm::mat4 &world, const Texture2D &tex)
{
	renderDisc(world, tex);
	renderCone(world, tex);
}
...
void CMyApp::Render()
{
	...
	//renderDisc(glm::rotate<float>(t * 2 * M_PI, glm::vec3(sin(t / 3), cos(t / 2), 0)), m_textureEarth);

	//renderDisc(glm::rotate<float>(t * 2 * M_PI, glm::vec3(1,0,0)), m_textureEarth);



	//renderCone(glm::translate(glm::vec3(0, 5, 0)), m_textureWall);
	//renderClosedCone(glm::rotate<float>(t * 2 * M_PI, glm::vec3(sin(t / 3), cos(t / 2), 0)), m_textureWall);

	//renderHalfSphere(glm::scale(glm::vec3(5,3,5)), m_textureShell);
	renderClosedHalfSphere(glm::translate(glm::vec3(0,0,0))*glm::rotate<float>(M_PI/4, glm::vec3(0,0,1)), m_textureShell);
	renderClosedHalfSphere(glm::rotate<float>(3*M_PI / 4, glm::vec3(0, 0, 1)), m_textureShell);


	
	//renderCylinder(glm::rotate<float>(t * 2 * M_PI, glm::vec3(sinf(t / 3), cosf(t / 2), 0))*glm::scale(glm::vec3(2,5,2)), m_textureShell);
					
	//renderDisc(glm::rotate<float>(t * 2 * M_PI, glm::vec3(sinf(t / 3), cosf(t / 2), 0))*
		//glm::rotate<float>(M_PI, glm::vec3(0,0,1))*
			//glm::scale(glm::vec3(2, 5, 2)), m_textureShell);
