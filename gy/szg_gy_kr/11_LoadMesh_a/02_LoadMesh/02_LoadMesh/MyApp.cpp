...
void CMyApp::DrawGround(const glm::mat4& world)
{
	...
	*/
	...
	glm::mat4 m_matWorldIT = glm::transpose(glm::inverse(world));
...
void CMyApp::DrawMesh(const glm::mat4& world)
{
...
void CMyApp::Render()
{
	...
	float t = SDL_GetTicks() / 1000.0 * 2 * M_PI / time;

	ImGui::SliderInt("n", &n, 1, 20);
	ImGui::SliderFloat("r", &r, 0.5f, 20.f);
	ImGui::SliderFloat("time", &time, 0.5f, 20.f);

	for (int i = 0; i < n; ++i) {
		float alpha = 2 * M_PI*i / (float)n;

		glm::mat4 w =

			glm::rotate(t, glm::vec3(0, 1, 0))*
			glm::translate(glm::vec3(r*cos(alpha), 2, r*sin(alpha)))
			;
		DrawMesh(w);
	}
