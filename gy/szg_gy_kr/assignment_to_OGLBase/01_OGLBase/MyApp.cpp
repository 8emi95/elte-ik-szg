...
void CMyApp::DrawCube(glm::mat4 worldTransform)
{
	m_program.SetUniform("MVP", m_camera.GetViewProj()*worldTransform);
	glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, nullptr);
}

void CMyApp::DrawTetris(std::vector<glm::vec3> tetrisPositions, std::vector<glm::vec3> cubePositions)
{
	m_vao.Bind();
	m_program.Use();

	glm::mat4 cubeWorld = glm::scale(glm::vec3(-1, -1, -1));
	
	for (auto tetrisPos : tetrisPositions)
	{
		for (auto cubePos : cubePositions)
		{
			...
			float v;
			if (speedUp) v = 2;
			else if (slowDown) v = 0.5;
			else v = 1;

			DrawCube(
				glm::rotate<float>(2*M_PI * v * t/5, glm::vec3(0,1,0)) * 
				glm::translate(tetrisPos) * 
				glm::translate(glm::vec3(0, 5*sinf(t), 0)) *
					glm::translate(cubePos) * 
									cubeWorld);
		}
	}
}
void CMyApp::Render()
{
	...
	std::vector<glm::vec3> cubePositions =	{ glm::vec3(-2,2,0), glm::vec3(0,2,0), glm::vec3(0,0,0), glm::vec3(0,-2,0), glm::vec3(2,-2,0) };
	
	std::vector<glm::vec3> tetrisPositions = { glm::vec3(10,0,0), glm::vec3(0,0,-10), glm::vec3(-10,0,0), glm::vec3(0,0,10) };

	DrawTetris(tetrisPositions, cubePositions);
