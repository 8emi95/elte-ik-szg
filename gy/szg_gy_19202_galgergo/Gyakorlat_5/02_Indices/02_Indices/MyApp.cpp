...
bool CMyApp::Init()
{
	...
	{ 
		...
		{glm::vec3(0, sqrt(2.0f), 0), glm::vec3(0, 1, 1)}
	};
...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3( 25,  10,  25),		// honnan nézzük a színteret
							glm::vec3( 0,  0,  0),		// a színtér melyik pontját nézzük
							glm::vec3( 0,  1,  0));		// felfelé mutató irány a világban
}


void CMyApp::DrawOctahedron(glm::mat4 trafo)
{
	m_matWorld = trafo * glm::mat4(1.0f);
	...
	glDrawElements(GL_TRIANGLES,		// primitív típus
		6 * 3,				// hany csucspontot hasznalunk a kirajzolashoz
...
void CMyApp::Render()
{
	...
	*/
	...
	// x-tengely körüli fogatás
	//glm::mat4 trafo = glm::rotate<float>(time, glm::vec3(1, 0, 0));

	/*
	glm::mat4 trafo =
		glm::translate(glm::vec3(2, 0, -2)) *
		glm::rotate<float>(time, glm::vec3(0, 1, 0));
	DrawOctahedron(trafo);
	...
	*/
	// Nap
	glm::mat4 trafo = glm::rotate<float>(2*M_PI*t/3,glm::vec3(0,1,0))*glm::scale(glm::vec3(2, 2, 2));
	DrawOctahedron(trafo);
	// Föld
	trafo = 
		glm::rotate<float>(2*M_PI*t/20, glm::vec3(0, 1, 0)) *
		glm::translate(glm::vec3(10, 0, 0))*
		glm::rotate<float>(2 * M_PI * t / 2, glm::vec3(0, 1, 0));
	DrawOctahedron(trafo);
