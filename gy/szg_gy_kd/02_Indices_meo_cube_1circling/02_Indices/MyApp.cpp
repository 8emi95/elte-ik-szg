...
const float getTime(){
	const float i = SDL_GetTicks() / 1000.f;
	return i;
}
...
bool CMyApp::Init()
{
	...
	{ 
		...
		{glm::vec3( -1,  -1, 2), glm::vec3(1, 0, 1)},
		{glm::vec3( 1,  -1, 2), glm::vec3(0, 1, 1)},
...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3( 0,  -10,  5),		// honnan nézzük a színteret
...
void CMyApp::Render()
{
	...
	*/
	...
	if (b) {
		actual_time = SDL_GetTicks() / 1000.f;
	}
	else {
		actual_time = getTime();
	}
	...
	//forgatás
	m_matWorld = glm::rotate<float>(2 * M_PI * actual_time / 10, glm::vec3(0.f, 0.f, 1.f)) * glm::translate<float>(glm::vec3(5.f, 0.f, 0.f));
...
{
	if (key.keysym.sym == SDLK_SPACE && b) {
		b = false;
	}
	else if (key.keysym.sym == SDLK_SPACE && !b) {
		b = true;
	}
}
