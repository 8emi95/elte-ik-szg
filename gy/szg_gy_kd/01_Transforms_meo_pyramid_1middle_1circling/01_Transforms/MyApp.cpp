...
//Feladat: A piramis a saját tengelye körül 5 másodperc alatt forduljon körbe a kör tenegelye körül pedig 10 másodperc alatt + egérmozgatás vagy billentyûzettel fel le mozgatni
CMyApp::CMyApp(void)
{
	m_vaoID = 0;
	m_vboID = 0;
...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3(0, -8, 5),		// honnan nézzük a színteret
	...
	m_matView = glm::lookAt(glm::vec3(cos(2 * M_PI * t),  sin(2 * M_PI * t),  5),		
							glm::vec3( 0,  0,  0),		
							glm::vec3( 0,  1,  0));		
	*/
}
...
void CMyApp::Render()
{
	...
	*/
	...
	//Forgatás kör tengelye körül
	m_matWorld = glm::rotate<float>(2 * M_PI * t / 10, glm::vec3(0.f, 0.f, 1.f)) * glm::translate<float>(glm::vec3(5.f, 0.f, 0.f));
	...
	//Eltolás
	//m_matWorld = glm::translate<float>(glm::vec3(0.5, 0.5, 0.5));
	//Forgatás saját tengelye körül
	m_matWorld = glm::rotate<float>(2 * M_PI * t / 5, glm::vec3(0.f, 0.f, 1.f));
