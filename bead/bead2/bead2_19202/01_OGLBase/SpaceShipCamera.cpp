...
void SpaceShipCamera::KeyboardDown(SDL_KeyboardEvent& key)
{
	...
	*/
	...
	case SDLK_SPACE:
		...
		/*
		irányvektor iv, sebességvektor sv

		v * -1 -> ezt kell hozzáadni ???
		sv hossza (gyök alatt négyzetösszeg) * 0.35 -> ezt levonni
		iv - normalizást 1 hosszúságú vektor -> iv-t adja vissza 

		a = jelenelgi sv hosszúnak 35%-a (sebesség 35%-a)
		b = sv normalize x,y vektor (iv)
		c = b*a (v ugyanoda mutat mint sv, de eredeti hossz 35%-a)

		ellentétes irányú vektort kell hozzáadni, kivonni c vektort (s = s - c)
