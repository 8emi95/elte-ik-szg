...
bool CMyApp::Init()
{
	...
	*/
	...
	std::vector<glm::vec3>	planeNorm;
	std::vector<glm::vec2>	planeText;
	std::vector<int>		planeIndices;


	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			planePos.push_back(glm::vec3( i - N/2, 0, j - N/2));
			planeNorm.push_back(glm::vec3(0, 1, 0));
			planeText.push_back(glm::vec2(i/N, j/N));
		}
	}
	...
	for (int i = 0; i<N; ++i)
		for (int j = 0; j<N; ++j)
		{
			...
			//
			planeIndices.push_back((i)		+ (j)		* (N + 1));
			planeIndices.push_back((i + 1)	+ (j)		* (N + 1));
			planeIndices.push_back((i)		+ (j + 1)	* (N + 1));
			planeIndices.push_back((i + 1)	+ (j)		* (N + 1));
			planeIndices.push_back((i + 1)	+ (j + 1)	* (N + 1));
			planeIndices.push_back((i)		+ (j + 1)	* (N + 1));

			
		}

	for (int v : planeIndices) {
		std::cout << v << std::endl;
	}

	// pozíciói
	m_gpuBufferPos.BufferData( planePos );

	// normálisai
	m_gpuBufferNormal.BufferData( planeNorm );

	// textúrakoordinátái
	m_gpuBufferTex.BufferData( planeText );

	// és a primitíveket alkotó csúcspontok indexei (az előző tömbökből) - triangle list-el való kirajzolásra felkészülve
	m_gpuBufferIndices.BufferData(planeIndices);
...
void CMyApp::Render()
{
	...
	m_program.SetUniform("t", t);

	m_vao.Bind();
	glDrawArrays(GL_LINE_STRIP, 0, N*N);
