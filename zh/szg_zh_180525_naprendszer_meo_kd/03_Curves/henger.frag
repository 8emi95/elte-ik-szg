...
uniform vec4 La = vec4(1.f, 1.f, 1.f, 1);
uniform vec4 Ld = vec4(1.f, 1.f, 1.f, 1);
uniform vec4 Ls = vec4(1, 1, 1, 1);

// anyagtulajdonságok
uniform vec4 Ka = vec4(1, 1, 1, 0);
uniform vec4 Kd = vec4(1.f, 1.f, 1.f, 1);
...
void main()
{
	...
	*/
	...
	fs_out_col = (ambient + diffuse + specular ) * texture(texImage, vs_out_tex0.st).bgra;
}