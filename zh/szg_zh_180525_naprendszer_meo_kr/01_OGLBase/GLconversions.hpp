...
template <typename T>			constexpr GLsizei ContainerSizeInBytes(const std::vector<T>& pArr) 
{ 
	return (GLsizei)(pArr.size()*sizeof(T)); 
}
...
template <typename T>			constexpr const T* PointerToStart(const std::vector<T>& pArg) 
{ 
