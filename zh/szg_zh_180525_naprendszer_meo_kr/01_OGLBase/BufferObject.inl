...
{
	Bind();
	auto t = static_cast<GLenum>(target);
	auto u = static_cast<GLenum>(usage);
	glBufferData(t, sizeof(GLushort)*indices.size(), indices.data(), u);
	m_sizeInBytes = pSize;
}
...
{
	BufferData(ContainerSizeInBytes(pArr), (const void*)(pArr.data()));
}
...
inline IsContiguousContainer<T> BufferObject<target, usage>::BufferData(const T & pArr)
{
	...
	auto t = static_cast<GLenum>(target);
	auto u = static_cast<GLenum>(usage);
	glBufferData(t, sizeof(T::value_type)*pArr.size(), pArr.data(), u);
	m_sizeInBytes = sizeof(T::value_type)*pArr.size();
}
