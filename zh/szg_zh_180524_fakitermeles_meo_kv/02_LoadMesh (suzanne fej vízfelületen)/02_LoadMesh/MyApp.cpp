...
bool CMyApp::Init()
{
	...
	// textúra betöltése
	m_waterTextureID = TextureFromFile("grass.jpg");
	m_samplerID = genSampler();

	// mesh betoltese
	m_mesh = ObjParser::parse("henger.obj");
	m_treebottomTextureID= TextureFromFile("bark.jpg");
	m_samplerID = genSampler();
...
void CMyApp::DrawMesh()
{
	...
	glActiveTexture(GL_TEXTURE0 + 0 /*hw samplerindex*/);
	// aktiváljuk a generált textúránkat 
	glBindTexture(GL_TEXTURE_2D, m_treebottomTextureID);
