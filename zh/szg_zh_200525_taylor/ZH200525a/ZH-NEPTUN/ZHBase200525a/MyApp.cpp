...
void CMyApp::InitCube()
{
	...
	for (int i = 0; i < 199; i++)
	{
		for (int j = 0; j < 199; j++)
		{
			vertices[ i      * 201 +  j     ] = { glm::vec3(-20.0 +  (float)i      * padding, 0, 20.0 -  (float)j      * padding), glm::vec3(0, 1, 0), glm::vec2((i + 20), (j + 20)) };
			vertices[(i + 1) * 201 +  j     ] = { glm::vec3(-20.0 + (float)(i + 1) * padding, 0, 20.0 -  (float)j      * padding), glm::vec3(0, 1, 0), glm::vec2((i + 20), (j + 20)) };
			vertices[(i + 1) * 201 + (j + 1)] = { glm::vec3(-20.0 + (float)(i + 1) * padding, 0, 20.0 - (float)(j + 1) * padding), glm::vec3(0, 1, 0), glm::vec2((i + 20), (j + 20)) };
			vertices[ i      * 201 + (j + 1)] = { glm::vec3(-20.0 +  (float)i      * padding, 0, 20.0 - (float)(j + 1) * padding), glm::vec3(0, 1, 0), glm::vec2((i + 20), (j + 20)) };
...
void CMyApp::Render()
{
	...
	*/
	...
	m_MeshProgram.SetUniform("worldIT", glm::inverse(glm::transpose(world)));
	m_CubeVao.Bind();
	glDrawElements(GL_TRIANGLES, 6*200*200, GL_UNSIGNED_INT, nullptr);
	m_CubeVao.Unbind();
